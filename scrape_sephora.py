...
class SephoraDataCollector:
    def __init__(self):
        ...
        #url to return all products in fragrance category
        self.product_url = 'https://www.sephora.com/api/catalog/categories/cat160006/products?currentPage={current_page}&pageSize=999999999&content=true&includeRegionsMap=true'
    ...
    def fetch_and_write_data(self, file_path='sephora_review_db.csv'):
        ...
        #column names in csv
        key_fields = ['brand', 'name', 'brand_id', 'brand_image_url', 'product_id', 'product_image_url', 'rating', 'incentivized_review',
                      'age', 'beauty_insider', 'user_name', 'review_text','price', 'recommended', 'first_submission_date', 'last_submission_date', 'location', 'description']
        ...
        with open(file_path, 'w', newline='') as csvfile:
            ...
            for p in progressbar.progressbar(product_ids, prefix='fetching reviews :: '):
                batch_review = self._fetch_all_reviews(p)
                for batch in batch_review:
                    writer.writerow(batch)
                    
    
    def fetch_and_print_tags(self, file_path = 'Sephora_Fragrance_Tags.csv'):
        ...
        df = pd.DataFrame()
        for p in progressbar.progressbar(product_ids, prefix= 'fetching tags :: '):
